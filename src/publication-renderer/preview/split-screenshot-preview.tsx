...
import { SplitScreenshotCard } from "../archetypes/split-screenshot/split-screenshot-card";
...
export function SplitScreenshotPreview({
  publication,
  persistFinalRender,
}: {
  ...
  useEffect(() => {
    ...
    const updateScale = () => setScale(Math.min(1, viewport.clientWidth / CANVAS_WIDTH));
    updateScale();
  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm font-semibold">Split Screenshot · v1</p>
          <p className="mt-1 text-xs text-[var(--muted)]">
            Pantalla y explicación comparten protagonismo en una sola pieza.
          </p>
        </div>
      ...
      >
        ...
        >
          <div ref={exportNodeRef} style={{ width: CANVAS_WIDTH, height: CANVAS_HEIGHT }}>
            <SplitScreenshotCard publication={publication} />
          </div>
        </div>
      </div>
    </div>
