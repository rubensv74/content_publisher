...
import { generateAndPersistSuggestions } from "./generate";
...
export async function generateSuggestionsAction(_formData: FormData) {
  const result = await generateAndPersistSuggestions();
...
export async function acceptSuggestionAction(formData: FormData) {
  ...
  if (error) {
    throw new Error(`No se pudo aceptar la sugerencia: ${error.message}`);
  }
...
export async function dismissSuggestionAction(formData: FormData) {
  ...
  if (error) {
    throw new Error(`No se pudo descartar la sugerencia: ${error.message}`);
  }
...
export async function convertSuggestionToIdeaAction(formData: FormData) {
  ...
  if (ideaError || !idea) {
    throw new Error(`No se pudo crear la Idea: ${ideaError?.message ?? "respuesta vacía"}`);
  }
