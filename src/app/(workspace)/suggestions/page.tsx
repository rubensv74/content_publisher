    ...
    <div className="mx-auto max-w-7xl">
      <header className="mb-8 flex flex-wrap items-end justify-between gap-5">
        <div className="max-w-3xl">
          ...
          <p className="mt-4 text-base leading-7 text-[var(--muted)]">
            Content Publisher prepara el contexto y ChatGPT Plus realiza el
            análisis de forma asistida. No existe una API de IA ni un coste
            adicional de proveedor dentro de la aplicación.
          </p>
        </div>
        <a
          href="/suggestions/chatgpt-packet"
      ...
      {Number.isFinite(imported) ? (
        <section className="mb-6 rounded-2xl border border-emerald-200 bg-emerald-50/60 p-5 text-sm text-emerald-950">
          Se importaron {imported} propuestas desde el flujo asistido con ChatGPT.
        </section>
      ) : null}

      {importError ? (
        <section className="mb-6 rounded-2xl border border-rose-200 bg-rose-50/70 p-5 text-sm leading-6 text-rose-950">
          <p className="font-semibold">No se pudo importar la respuesta.</p>
          <p className="mt-1">{importError}</p>
          <p className="mt-2 text-rose-800">
            Puedes corregir el JSON o guardar la respuesta como .json/.txt y
            seleccionarla directamente abajo.
          </p>
        </section>
      ) : null}
      ...
      <section className="mb-6 grid gap-4 md:grid-cols-4">
        <div className="rounded-2xl border border-[var(--border)] bg-white p-5">
          ...
          <p className="mt-1 text-sm text-[var(--muted)]">señales pendientes</p>
        </div>
        <div className="rounded-2xl border border-[var(--border)] bg-white p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">
            Pendientes
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-white p-5">
          ...
          <p className="mt-1 text-sm text-[var(--muted)]">
            listas para convertirse en Idea
          </p>
        </div>
        <div className="rounded-2xl border border-blue-200 bg-blue-50/60 p-5">
          ...
          <p className="mt-2 text-lg font-semibold">ChatGPT Plus · manual</p>
          <p className="mt-1 text-sm leading-6 text-[var(--muted)]">
            Sin OPENAI_API_KEY y sin llamadas de pago desde la app.
          </p>
        </div>
      </section>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          <div className="rounded-xl bg-slate-50 p-4">
            ...
            <p className="mt-2 text-sm leading-6 text-[var(--muted)]">
              Descarga el TXT. La app refresca las señales y añade contexto
              técnico limitado y sanitizado.
            </p>
          </div>
          <div className="rounded-xl bg-slate-50 p-4">
            <p className="font-semibold">2 · Analizar</p>
            <p className="mt-2 text-sm leading-6 text-[var(--muted)]">
              Abre ChatGPT Plus, adjunta el TXT y sigue las instrucciones que
              contiene. ChatGPT devolverá únicamente JSON.
            </p>
          </div>
          <div className="rounded-xl bg-slate-50 p-4">
            <p className="font-semibold">3 · Importar</p>
            <p className="mt-2 text-sm leading-6 text-[var(--muted)]">
              Pega el JSON o sube la respuesta como archivo .json/.txt.
              Content Publisher valida el contrato antes de guardar nada.
            </p>
          </div>
        </div>

        <form
          action={importChatGPTSuggestionsAction}
          encType="multipart/form-data"
          className="mt-5"
        >
          <label htmlFor="suggestionsJson" className="text-sm font-semibold">
          ...
          <label htmlFor="suggestionsFile" className="text-sm font-semibold">
            Archivo de respuesta
          </label>
          <input
            id="suggestionsFile"
            name="suggestionsFile"
            type="file"
            accept=".json,.txt,application/json,text/plain"
            className="mt-2 block w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-600 file:mr-4 file:rounded-lg file:border-0 file:bg-slate-900 file:px-3 file:py-2 file:text-sm file:font-semibold file:text-white"
          />

          <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
            <p className="text-xs text-slate-500">
              Se aceptan .json y .txt de hasta 256 KB. Si pegas texto y además
              seleccionas archivo, se utiliza el texto pegado. La importación
              rechaza IDs inexistentes y valores fuera del contrato.
            </p>
            <SubmitButton
      ...
      {suggestions.length === 0 ? (
        <section className="rounded-2xl border border-dashed border-slate-300 bg-white/60 p-8 text-center">
          ...
          <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-[var(--muted)]">
            Descarga un paquete, procésalo en ChatGPT Plus e importa el JSON
            resultante.
          </p>
          <Link
            href="/signals"
              ...
              <div className="flex flex-wrap items-start justify-between gap-5">
                <div className="max-w-4xl">
                  <div className="flex flex-wrap items-center gap-2">
                    <span
                      className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${statusClass(suggestion.status)}`}
                    >
                ...
                <div className="min-w-52 text-right text-xs text-slate-500">
                  ...
                  <p className="mt-1">
                    {suggestion.designFamily} · {suggestion.archetypeKey}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {suggestion.status === "converted" ? (
                    <Link
                      href="/ideas"
                      className="rounded-xl border border-violet-200 bg-violet-50 px-3.5 py-2 text-sm font-semibold text-violet-700"
                    >
                      Ver en Ideas
                    </Link>
                  ) : null}
